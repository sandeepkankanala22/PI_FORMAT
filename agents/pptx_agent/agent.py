...
C_PURPLE   = RGBColor(0x7C, 0x3A, 0xED)   # #7C3AED  product-share accent
...
C_PEAK_TINT = RGBColor(0xEB, 0xF3, 0xFC)  # light-blue peak/hero-row tint
...
def _rect(slide, left, top, w, h, fill: RGBColor | None = None, line: RGBColor | None = None, line_width=Pt(0.5)):
...
# Built via python-pptx's chart API (real, editable PPT chart objects — not images).
...
def _style_chart_base(chart, legend=True):
    ...
    if legend:
        ...
        chart.legend.font.color.rgb = C_SUBTEXT
    else:
        chart.has_legend = False
    cat_ax = chart.category_axis
    cat_ax.tick_labels.font.size = Pt(9)
    cat_ax.tick_labels.font.color.rgb = C_SUBTEXT
    cat_ax.format.line.color.rgb = C_MIDGREY
    cat_ax.has_major_gridlines = False
    val_ax = chart.value_axis
    val_ax.tick_labels.font.size = Pt(9)
    val_ax.tick_labels.font.color.rgb = C_SUBTEXT
    val_ax.format.line.color.rgb = C_MIDGREY
    val_ax.has_major_gridlines = True
    val_ax.major_gridlines.format.line.color.rgb = RGBColor(0xE0, 0xE6, 0xED)
    val_ax.major_gridlines.format.line.width = Pt(0.5)
    return cat_ax, val_ax


def _add_chart_revenue(slide, left, top, w, h, years, gross, net):
    data = CategoryChartData()
    data.categories = [str(y) for y in years]
    data.add_series("Gross Sales ($M)", gross)
    data.add_series("Net Sales ($M)", net)
    chart = slide.shapes.add_chart(XL_CHART_TYPE.COLUMN_CLUSTERED, left, top, w, h, data).chart
    _style_chart_base(chart)
    for series, color in zip(chart.series, (C_PRIMARY, C_GOLD)):
...
def _add_chart_patients(slide, left, top, w, h, years, patients):
    data = CategoryChartData()
    data.categories = [str(y) for y in years]
    data.add_series("Treated Patients", patients)
    chart = slide.shapes.add_chart(XL_CHART_TYPE.LINE_MARKERS, left, top, w, h, data).chart
    _style_chart_base(chart, legend=False)
    series = chart.series[0]
    series.format.line.color.rgb = C_GREEN
    series.format.line.width = Pt(2.75)
    series.marker.style = XL_MARKER_STYLE.CIRCLE
    series.marker.size = 8
    series.marker.format.fill.solid()
    series.marker.format.fill.fore_color.rgb = C_WHITE
    series.marker.format.line.color.rgb = C_GREEN
    series.smooth = False
    plot = chart.plots[0]
    plot.has_data_labels = True
    dl = plot.data_labels
    ...
    dl.font.size = Pt(8.5)
    dl.font.bold = True
    dl.font.color.rgb = C_GREEN
    return chart


def _add_chart_shares(slide, left, top, w, h, years, cls, prod):
    data = CategoryChartData()
    data.categories = [str(y) for y in years]
    data.add_series("Class Share (%)", [float(v) for v in cls])
    data.add_series("Product Share (%)", [float(v) for v in prod])
    chart = slide.shapes.add_chart(XL_CHART_TYPE.LINE_MARKERS, left, top, w, h, data).chart
    _style_chart_base(chart)
    for series, color in zip(chart.series, (C_PRIMARY, C_GOLD)):
...
def _cell_style_multiline(cell, lines, sizes, bolds, colors,
                           bg: RGBColor = C_WHITE, align=PP_ALIGN.CENTER):
    """Like _cell_style but writes several stacked paragraphs (e.g. value + a small badge line)."""
    cell.fill.solid()
    cell.fill.fore_color.rgb = bg
    tf = cell.text_frame
    tf.clear()
...
def _add_forecast_table(slide, results, left, top, w, h):
    """Year-by-year results table (transposed: years as columns, metrics as rows).

    Mirrors the web results table's visual language: each metric row carries its
    own accent color (label text). "Peak" is a Net Sales concept, so the peak-year
    emphasis (tint + badge) is scoped to the Net Sales ("hero") row only — other
    rows (several of which are flat, like Cost/Pt and Discount) don't get a
    misleading "peak" treatment just because they share that column.
    """
    if not results:
        return

    # label -> (value fn, accent color)
    metrics = [
        ("Eligible Pts", lambda r: f"{int(r.get('eligiblePatients', 0) or 0):,}", C_BLUE2),
        ("Class %",      lambda r: f"{r.get('classShare', 0)}%", C_TEAL),
        ("Prod %",       lambda r: f"{r.get('productShare', 0)}%", C_PURPLE),
        ("Treated Pts",  lambda r: f"{int(r.get('treatedPatients', r.get('productPatients', 0)) or 0):,}", C_GREEN),
        ("Cost / Pt",    lambda r: f"${r.get('annualCost', '150,000')}", C_ORANGE),
        ("Gross ($M)",   lambda r: f"${float(r.get('grossSales', 0) or 0):.1f}", C_PRIMARY),
        ("Disc %",       lambda r: f"{r.get('discount', 0)}%", C_SUBTEXT),
        ("Net ($M)",     lambda r: f"${float(r.get('netSales', 0) or 0):.1f}", C_PRIMARY),
    ]
    HERO_LABEL = "Net ($M)"

    num_years = len(results)
    num_cols = num_years + 1
    num_rows = len(metrics) + 1  # header row + metrics rows

    tbl_shape = slide.shapes.add_table(num_rows, num_cols, left, top, w, h)
    tbl = tbl_shape.table

    # Column widths
    first_col_pct = 0.18
    remaining_pct = (1.0 - first_col_pct) / num_years
    col0_w = int(w * first_col_pct)
    col_w  = int(w * remaining_pct)
    tbl.columns[0].width = col0_w
    for ci in range(1, num_cols):
        tbl.columns[ci].width = col_w

    # Peak (highest net sales) column — index into results / column index (1-based)
    max_net = max(float(r.get("netSales", 0) or 0) for r in results)
    peak_ci = next((ci for ci, row in enumerate(results, 1)
                     if float(row.get("netSales", 0) or 0) == max_net), None)

    # Row 0: Header row (Metric + years) — plain, no column-wide peak styling
    _cell_style(tbl.cell(0, 0), "Metric", size=8.5, bold=True,
                fg=C_WHITE, bg=C_PRIMARY, align=PP_ALIGN.CENTER)
    for ci, row in enumerate(results, 1):
        year_str = str(row.get("year", ""))
        _cell_style(tbl.cell(0, ci), year_str, size=8.5, bold=True,
                    fg=C_WHITE, bg=C_PRIMARY, align=PP_ALIGN.CENTER)

    # Rows 1..N: metric values, each in its own accent color
    for ri, (label, get_val, accent) in enumerate(metrics, 1):
        is_hero = label == HERO_LABEL
        row_bg = C_PEAK_TINT if is_hero else (C_LTGREY if ri % 2 == 0 else C_WHITE)

        _cell_style(tbl.cell(ri, 0), label, size=8.5, bold=True,
                    fg=accent, bg=row_bg, align=PP_ALIGN.LEFT)

        for ci, row in enumerate(results, 1):
            val = get_val(row)
            if is_hero and ci == peak_ci:
                _cell_style_multiline(tbl.cell(ri, ci), [val, "PEAK"],
                                       [9, 7], [True, True], [accent, accent],
                                       bg=row_bg, align=PP_ALIGN.RIGHT)
            else:
                _cell_style(tbl.cell(ri, ci), val, size=8.5, bold=is_hero,
                            fg=accent if is_hero else C_TEXT,
                            bg=row_bg, align=PP_ALIGN.RIGHT)
...
def generate_forecast_pptx(user_input: dict, template_path: str, output_path: str) -> str:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # SLIDES 6-8 — Charts (Blank, manual title bar + native PPT chart)
    # ══════════════════════════════════════════════════════════════════════════
    # Map each chart to its builder fn + AI narrative
    chart_defs = []
    if years and gross_s:
        chart_defs.append(("Revenue Forecast ($M)",
                            lambda s, l, t, w, h: _add_chart_revenue(s, l, t, w, h, years, gross_s, net_s),
                            n_rev))
    if years and treated:
        chart_defs.append(("Patient Volume",
                            lambda s, l, t, w, h: _add_chart_patients(s, l, t, w, h, years, treated),
                            n_pat))
    if years and cls_share:
        chart_defs.append(("Market Share Ramp (%)",
                            lambda s, l, t, w, h: _add_chart_shares(s, l, t, w, h, years, cls_share, prod_share),
                            ""))

    for title, build_chart, chart_narr in chart_defs:
        slide = prs.slides.add_slide(L_BLANK)
        _title_bar(slide, title, W, H)
        chart_strip_top = _commentary_strip(slide, chart_narr, W, H,
                                            label="KEY INSIGHT",
                                            strip_h=Inches(0.68))
        chart_top = BAR_H + ACC_H + Inches(0.10)
        chart_h   = chart_strip_top - chart_top - Inches(0.08)
        build_chart(slide, PAD_L, chart_top, W - PAD_L - PAD_R, chart_h)
